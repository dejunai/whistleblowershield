...
add_filter( 'post_row_actions', 'ws_filter_legal_update_row_actions', 10, 2 );

// Remove bulk delete/trash actions on the Legal Updates list table.
add_filter( 'bulk_actions-edit-ws-legal-update', 'ws_filter_legal_update_bulk_actions' );

// If a blocked delete/trash attempt occurs, show a clear admin notice.
add_action( 'admin_notices', 'ws_render_legal_update_delete_block_notice' );

/**
 * Removes delete/trash row actions for legal updates in the list table.
 *
 * @param array        $actions Existing row actions.
 * @param WP_Post|null $post    Current post row object.
 * @return array
 */
function ws_filter_legal_update_row_actions( $actions, $post ) {
    if ( $post instanceof WP_Post && $post->post_type === 'ws-legal-update' ) {
...
/**
 * Removes bulk delete/trash actions for the legal updates list table.
 *
 * @param array $actions Existing bulk actions.
 * @return array
 */
function ws_filter_legal_update_bulk_actions( $actions ) {
    unset( $actions['trash'], $actions['delete'] );
    return $actions;
}

/**
 * Displays an admin notice after a legal update delete attempt is blocked.
 *
 * @return void
 */
function ws_render_legal_update_delete_block_notice() {
